...
const CategoriesList = ({categories,selectedCategory,setSelectedCategory}:Props) => {
  return (
            ...
            <RadioGroup value={selectedCategory || ""}
             ...
             {categories?.map((category)=>(
                ...
                <Label 
                  ...
                  className={`${selectedCategory === category?.slug?.current ? "font-semibold text-shop_dark_green":"font-normal"}`}>{category?.title}</Label>
                </div>
             ))}
              {selectedCategory && (
                <button 
